...
def losCampos(campo, entidad):
    campoID = campo.split('.')[0]
    resultado = [campoID]
    print "param ", campo
    
    definicionCampo = campos[entidad][campoID]
    #checamos si el campo tiene campos subsidiarios.
    #es el caso de notas de municipio
    #en ese caso queremos un checkbox especifico para incluir o no las notas
    if len(definicionCampo) > 3:
        for i in definicionCampo[3]:
            resultado.append(i)
    return resultado
...
def camposTree(mat, entidad, NB, init=False, tipoRep="normal"):
    ...
    n = 0
    print "mat[entidad]", mat[entidad]
    for campoID in mat[entidad]: 
        misCampos = losCampos(campoID, entidad)
        print "mis campos", misCampos
        for miCampo in misCampos:
            #para cada campo imprimible de la entidad se agrega un checkbox
            #print "i1 ", campoID
            #miCampo = campoID.split('.')[0]
            id = wx.NewId()
...
def nivelCampo(entidad, campo):
    print "nivel campo", campos[entidad][campo]
